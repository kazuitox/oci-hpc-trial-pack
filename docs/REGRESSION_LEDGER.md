...
| REG-017 | Instance Pool・Cluster Network・Compute Clusterの実OSホスト名とOCI表示名・DNS・inventoryを整合させる。新規作成では名前同期成功後に残りの構築・Slurm起動へ進み、失敗時は同じノードの未完了段階から再開する。所属・所有権とTerraform管理ノードを保護し、部分失敗の記録・再試行・排他制御を維持する。 | [Instance Pool](../tests/test_resize_instance_pool_hostname_sync.py)、[Cluster Network](../tests/test_resize_cluster_network_hostname_sync.py)、[Compute Cluster](../tests/test_resize_compute_cluster_hostname_sync.py)：OCI等をモック化した同期・移行・削除の処理テスト。[構築順序と再開](../tests/test_configure_as_early_name_sync.py)：実Shellとコマンドスタブ。[段階記録](../tests/test_initial_configure_state.py)：対象OCID・IPの一致と原子的保存。[更新競合](../tests/test_resize_instance_name_conflict_retry.py)：限定409の回数・時間上限、他エラーと部分更新後の再開。REG-002 / REG-016と併せ、新規ノードの一時名とコストタグ初期化も確認する。 |
...
| REG-017 | 3種類の作成方式でOSホスト名とOCIインスタンス・Primary VNIC表示名を照合する。新規作成の通常版・軽量版で名前同期がSlurm起動より先に完了し、ジョブ開始時のコストタグ更新がその後に行われることを確認する。同期途中失敗では後半へ進まず、再開後に後半まで完了すること、後半・監視の失敗後に必要な段階だけ再開すること、別OCIDへの置換時に古い段階で進まないことを確認する。カスタムイメージの起動時slurmd状態も確認する。既存管理CLIの移行・対象外ノードと共有リソースの保護も維持する。 |
