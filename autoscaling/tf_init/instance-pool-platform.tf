# Query the launch AD separately from any regional image-compatibility lookup.
data "oci_core_shapes" "instance_pool_vm_shape" {
  count = !var.cluster_network && local.instance_pool_is_vm && !local.instance_pool_is_arm ? 1 : 0
